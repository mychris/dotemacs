...
#+begin_src emacs-lisp
  (use-package evil-easymotion
    ...
    (emacs-startup-hook . (lambda ()
                            (when (fboundp 'evilem-default-keybindings)
...
#+begin_src emacs-lisp
  ...
  TODO: needs refactoring."
    ...
    :general
    (emacs-map
      "H-f" #'myext-focus-minibuffer-or-completion
...
#+begin_src emacs-lisp
  (use-package consult
    ...
    (setq-default register-preview-function #'consult-register-format)
    :custom
...
#+begin_src emacs-lisp
  (use-package quick-file-access
    ...
    :general
    (emacs-map
      "<escape> <escape> o" #'quick-file-access-open-file))
#+end_src
* Visual
** whitespace [built-in]
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package hi-lock
    ...
    :general
    (emacs-map
      "M-s h ." #'+highlight-region-or-symbol
...
#+begin_src emacs-lisp
  (use-package evil-mc
    ...
    :commands (global-evil-mc-mode
               +global-evil-mc-mode-toggle)
    :init
    ;; (setq-default evil-mc-one-cursor-show-mode-line-text nil)
    :config
    (defun +global-evil-mc-mode-toggle ()
      "Enable/Disable `evil-mc-mode' globally in every buffer dependent an the
  status of the `evil-mode'."
      (interactive)
    ...
    :hook
    (evil-mode-hook . +global-evil-mc-mode-toggle))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package ligature
    ...
    :config
    (ligature-set-ligatures '(prog-mode emacs-lisp-mode) '("|||>" "<|||" "<==>" "<!--" "####" "~~>" "***" "||=" "||>"
                                       ":::" "::=" "=:=" "===" "==>" "=!=" "=>>" "=<<" "=/=" "!=="
                                       "!!." ">=>" ">>=" ">>>" ">>-" ">->" "->>" "-->" "---" "-<<"
                                       "<~~" "<~>" "<*>" "<||" "<|>" "<$>" "<==" "<=>" "<=<" "<->"
                                       "<--" "<-<" "<<=" "<<-" "<<<" "<+>" "</>" "###" "#_(" "..<"
...
#+begin_src emacs-lisp
  (use-package elisp-mode
    ...
    (emacs-lisp-mode
     ...
     ("pi" . ?π)
     ("use-package" . ?📦)
     ("lambda" . (?\s (Br . Bl) ?\s (Bc . Bc) ?λ))
     ("defun" . (?\s (Br . Bl) ?\s (Bc . Bc) ?ƒ)))
    :hook
