...
#+begin_src emacs-lisp
  ...
  (general-create-definer system-local-leader
    ...
    :keymaps 'override
    :prefix "SPC s"
    :non-normal-prefix "C-SPC s")
  (general-create-definer mode-action-leader
    :states '(normal motion visual insert)
    :prefix "SPC e"
    :non-normal-prefix "C-SPC e")
  ...
  (general-define-key
   ...
   "C-<backspace>" '(:keymap c-x-leader-map)
   "C-DEL" '(:keymap c-x-leader-map)
   "C-SPC" nil
   "C-SPC g" '(:keymap buffer-local-leader-map))
    ...
    (setq display-buffer-alist
        '(((or . ((derived-mode . occur-mode)
           ...
           (display-buffer-reuse-mode-window
            +display-buffer-right-or-below-selected)
           (dedicated . t)
           (body-function . +select-window-and-fit-to-buffer))
          )))
...
#+begin_src emacs-lisp
  (use-package which-key
    ...
    (which-key-add-key-based-replacements "C-SPC f" "link-hint prefix")
    ...
    (which-key-add-key-based-replacements "C-SPC g e" "next-error prefix")
    (which-key-add-key-based-replacements "SPC g h" "hi-lock prefix")
    (which-key-add-key-based-replacements "C-SPC g h" "hi-lock prefix")
    (which-key-add-key-based-replacements "C-SPC g" "buffer-local-leader")
    (which-key-add-key-based-replacements "SPC s" "system-local-leader")
    (which-key-add-key-based-replacements "C-SPC s" "system-local-leader")
    (which-key-add-key-based-replacements "SPC e" "mode-action-leader")
    (which-key-add-key-based-replacements "C-SPC e" "mode-action-leader")
    (which-key-add-key-based-replacements "SPC ," "inspect-leader")
    (which-key-add-key-based-replacements "C-SPC ," "inspect-leader")
    (which-key-add-key-based-replacements "SPC b" "bookmark prefix")
    (which-key-add-key-based-replacements "C-SPC b" "bookmark prefix")
    (which-key-add-key-based-replacements "SPC w" "window prefix")
    (which-key-add-key-based-replacements "C-SPC w" "window prefix")
    (which-key-add-key-based-replacements "SPC t" "tab prefix")
    (which-key-add-key-based-replacements "C-SPC t" "tab prefix")
...
#+begin_src emacs-lisp
  ...
  Otherwise, it is opened via `find-file'."
      ...
      (let ((target (dired-get-file-for-visit)))
        (if (file-directory-p target)
            (find-alternate-file (file-truename target))
          (find-file target))))
  ...
  (use-package dired-aux
    :ensure nil
    :after (:all dired)
    :commands (dired-maybe-insert-subdir
               dired-kill-subdir
               dired-hide-subdir
               dired-hide-all
               dired-do-touch
...
#+begin_src emacs-lisp
  ...
  (use-package naysayer-theme
    ...
    (custom-theme-set-faces
     ...
     `(tab-bar-tab ((t (:foreground "#062329" :background "#d1b897"))))
     `(tab-bar-tab-inactive ((t (:foreground "#d1b897" :background "#062329"))))))
...
#+end_src
** completion-preview [built-in] - Preview completion with inline overlay
#+begin_src emacs-lisp
  (use-package completion-preview
    ...
    :hook
    (company-mode-hook . completion-preview-mode))
#+end_src
...
#+begin_src emacs-lisp
  (use-package zig-mode
    :ensure t
    :config
    (defun +zig-mode-setup ()
      (setq-local compile-command "zig build")
      (setq-local indent-tabs-mode nil)
      (setq-local show-trailing-whitespace t))
    :hook
    (zig-mode-hook . +zig-mode-setup))
#+end_src
** Emacs-lisp
#+begin_src emacs-lisp
...
#+end_src
** ediff [built-in] - a comprehensive visual interface to diff & patch
#+begin_src emacs-lisp
  (use-package ediff
    :ensure nil
    :custom
...
#+begin_src emacs-lisp
  (use-package system-packages
    :ensure t)
#+end_src
** message [built-in] - composing mail and news messages
#+begin_src emacs-lisp
  (use-package message
    :ensure nil
    :defer t
    ...
    :config
    (defun +message-tab-function ()
      (interactive)
      (let ((p (point)))
        (when (re-search-forward "^--text follows this line--$" (point-max) t)
          (goto-char p)
          (forward-line 1)
          (beginning-of-line)
          (if (looking-at-p "^--text follows this line--$")
              (message-goto-body)
            (when (looking-at "^[!-~]+:[ \t]")
              (message "TEST")
            ...
            (when (not (= (point)
              ...
              (set-mark (point))
              (end-of-visual-line))))))
    (defun +message-backtab-function ()
      (interactive)
      nil)
    :custom
    ...
    (message-sendmail-extra-arguments '("--passwordeval=gpg --use-agent --quiet --batch -d ~/.msmtp-credentials.gpg")))
#+end_src
** sendmail [built-in] - mail sending commends for Emacs
#+begin_src emacs-lisp
...
#+end_src
** notmuch - run notmuch within emacs
#+begin_src emacs-lisp
  (use-package notmuch
    :ensure t
    :commands (notmuch
    ...
    (notmuch-poll-script "offlineimap")
    ...
    (notmuch-saved-searches
     '((:name "📥 inbox"          :query "tag:inbox"          :key "i"  :sort-order newest-first :search-type tree)
       ...
       (:name " emacs-devel"    :query "tag:emacs-devel"    :key "ed" :sort-order newest-first :search-type tree)
       (:name " emacs-tangents" :query "tag:emacs-tangents" :key "et" :sort-order newest-first :search-type tree)
       (:name " emacs-help"     :query "tag:emacs-help"     :key "eh" :sort-order newest-first :search-type tree)
       (:name " emacs-info"     :query "tag:emacs-info"     :key "ei" :sort-order newest-first :search-type tree)
       (:name " flagged"        :query "tag:flagged"        :key "f"  :sort-order newest-first :search-type unthreaded)
       (:name " sent"           :query "tag:sent"           :key "t"  :sort-order newest-first :search-type unthreaded)
       (:name " drafts"         :query "tag:drafts"         :key "d"  :sort-order newest-first :search-type unthreaded)
       (:name " archive"        :query "tag:archived"       :key "A"  :sort-order newest-first :search-type unthreaded)
