...
#+begin_src emacs-lisp
  (use-package which-key
    ...
    (which-key-add-key-based-replacements "M-g h" "hi-lock prefix")
...
#+begin_src emacs-lisp
  (use-package hi-lock
    ...
    (emacs-map
      'hi-lock-map
      :no-autoload t
      "M-g h i" #'hi-lock-find-patterns
      "M-g h l" #'highlight-lines-matching-regexp
      "M-g h p" #'highlight-phrase
      "M-g h h" #'highlight-regexp
      "M-g h r" #'unhighlight-regexp
      "M-g h b" #'hi-lock-write-interactive-patterns
...
#+begin_src emacs-lisp
  ...
  The element at that index will be used at the next call to `+font-cycle-step'.")
    ...
    (cond
     ...
     ((and (eq system-type 'gnu/linux) (daemonp) (getenv "DISPLAY"))
      ;; daemon running on GNU/Linux with X
      (add-hook 'server-after-make-frame-hook #'(lambda () (when windown-system (+initialize-fonts))))))
